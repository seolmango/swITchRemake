...
export interface InputState {
    ...
    down: boolean;
    /**
     * 누르고 있는 동안 유지되는 행동 비트.
     *
     * **지금은 예약 필드다. 정의된 비트가 없고 시뮬레이션도 읽지 않는다.** 스킬은 이 패킷이 아니라
     * `game.useSkill` JSON 메시지로 간다 — 스위치가 대상 지목(`targetPlayerId`)을 필요로 하는데
     * 6바이트 입력 패킷에 그걸 실을 자리가 없고, 스킬은 30Hz로 보낼 것도 아니기 때문이다.
     *
     * 자리를 비워 두는 이유는 "누르고 있는 동안"이 필요한 행동(예: 조준 유지)이 생기면 그때는 tick
     * 단위 정확도가 필요해서 이 패킷이 맞는 통로이기 때문이다. 그런 게 생기기 전까지 항상 0이다.
     */
    heldActions: number;
}
