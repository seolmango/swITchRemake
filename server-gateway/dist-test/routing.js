...
exports.resolveWebSocketBackend = resolveWebSocketBackend;
exports.resolveBundleBackend = resolveBundleBackend;
 ...
 */
function resolveWebSocketBackend(path, servers) {
    if (!path.startsWith(GAME_WS_PREFIX))
        return null;
    ...
    if (server === undefined || server.internalAddress.length === 0)
        return null;
 ...
 */
function resolveBundleBackend(path, servers) {
    if (!path.startsWith(MAP_BUNDLE_PREFIX))
        return null;
...
const backendLoad = (server) => server.waitingRooms + server.playingRooms + server.connections / 8 + server.loopLagMs / 1000;
