...
class RegistryView {
    ...
    constructor(options) {
        ...
        this.#refreshMs = options.refreshMs ?? shared_1.HEARTBEAT_INTERVAL_MS;
    }
    get servers() {
        return this.#servers;
    }
        ...
        try {
            ...
            const entries = await Promise.all(ids.map(async (id) => {
                ...
                try {
                    ...
                    return value.serverId === id ? [id, value] : null;
                }
                catch {
                    return null;
                }
            }));
            this.#servers = new Map(entries.filter((entry) => entry !== null));
        }
        catch (error) {
