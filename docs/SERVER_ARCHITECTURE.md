...
방 생성, 코드 참가, 빠른 참가, 재접속 API의 성공 응답은 모두
`{ roomId, wsPath, ticket, expiresAt }` 형태로 통일한다. 제어 평면의 `RESERVE_JOIN`과
`RESERVE_RESUME` 응답에는 `roomId`가 없으므로 매칭 서버가 자신이 배정한 방 ID를 외부 API 응답에
덧붙인다. 특히 빠른 참가는 요청 시점에 클라이언트가 방 ID를 모르므로 이 필드가 반드시 필요하다.
