...
- **`{alreadyAssigned:true, roomId}` 응답을 클라이언트가 무시한다.** 이미 방에 있는 사람이 코드 참가나 빠른
  참가를 누르면 서버가 이 응답과 함께 `roomId`를 주는데, 클라이언트가 아무 데도 안 가고 화면이 그대로 멈춘다.
  2026-08-24 브라우저 검증에서 재현했다. 그 `roomId`의 로비로 보내야 한다.
- **참가 빈도 제한이 로컬 테스트를 막는다.** 게스트 IP 기준 6회/분(`JOIN_RATE_LIMIT`)이라 탭을 몇 개
  띄우며 시험하면 금방 걸리고, 걸렸을 때 화면에 이유가 안 뜬다. 최소한 사유를 보여줘야 한다.
...
- ~~`COMMAND_TIMEOUT_MS = 2000`에 근접하는 왕복 지연의 원인~~ — **해결(403a040)**. 블로킹 Redis 읽기가
  일반 명령과 커넥션을 공유하고 있었다. 전용 커넥션으로 분리해 `GET /rooms` 6초 → 4ms가 됐다
- **매칭 서버 다중 인스턴스에서 제어 응답을 남이 가져간다.** 소비자 이름은 프로세스마다 다르지만
  (`matching-{uuid}`) 같은 소비자 그룹이라, 다른 인스턴스가 자기 `pending`에 없는 reply를 읽고 ACK하면
  원 요청 인스턴스는 operation-cache 복구 타이머(2초)까지 기다린다. 지금은 단일 인스턴스라 안 드러나지만
  수평 확장하는 순간 모든 방 생성이 2초가 된다. reply를 요청한 인스턴스에게만 보내거나(전용 스트림),
  남의 reply를 ACK하지 않고 남겨야 한다
